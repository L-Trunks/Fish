<template>
  <div style="padding: 20px">
    ...
    >
      ...
      <el-form-item class="sub_btn" style="width: 100%">
        <div style="margin-top: 80px">
          <el-button
            ...
            >提交</el-button
          >
          <el-button type="primary" @click="handleShow">预览</el-button>
        </div>
      </el-form-item>
    </el-form>
    ...
    getArticleInfo() {
      SelectArticleById({ articleId: this.articleid })
        .then((res) => {
          ...
          this.fileImgList = [{ name: "xx.jpg", url: res.data[0].img_url }];
          if(res.data[0].type == '视频')
          this.fileVideoList = [{ name: "xx", url: res.data[0].content }];
            ...
            .then((_) => {
              ...
              AddArticle(this.editForm)
                .then((res) => {
                  ...
                  // this.fileImgList = [];
                  // this.fileVideoList = [];
                  this.$message.success("更新成功，快去看看吧");
                })
                .catch((err) => {
                  this.$message.error("出现错误，请稍候再试");
                });
