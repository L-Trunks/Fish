        ...
        <div class="person_box">
          ...
          <div class="user_info_box">
            <span style="font-size: 24px; color: #fff; margin-left: 15px">{{
              (userInfo && userInfo.name) || "未知用户，请登录"
            }}</span>
            <span style="font-size: 12px; color: #999; margin: 10px 15px"
              >权限：{{
                permissionMap[(userInfo && userInfo.permission) || "0"]
              }}</span
            >
          </div>
        </div>
...
import { UpdateUserInfo, SelectUserByToken } from "../api/user_api";
...
export default {
  ...
  mounted() {
    ...
    if (
      (this.userInfo && this.userInfo.permission == "0") ||
      this.userInfo &&this.userInfo.permission == "1"
    ) {
